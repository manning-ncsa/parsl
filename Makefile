...
mypy: ## run mypy checks
	PYTHONPATH=.:$(PYTHONPATH) MYPYPATH=$(CWD)/mypy-stubs mypy --no-incremental \
	parsl/tests/configs/ \
	parsl/tests/sites/ \
	parsl/app/ parsl/channels/ parsl/dataflow/ parsl/data_provider/ parsl/launchers parsl/providers/ \
	typecheck-root.py parsl/executors/high_throughput/ parsl/executors/*.py parsl/*py parsl/monitoring/*py \
